...
(async () => {
    ...
    if (!window.hydrate.has('showYears')) {
        if (year) {
            window.hydrate.set('showYears', year.toString(), true);
        } else {
            window.hydrate.set('showYears', '9,10,11,12,13', true);
        }
    }
    
    await core.api(`get/users/leaderboard`).then(({ data }) => {
        window.hydrate.set('students', data);
        yearGroups(data);
    });
})();
